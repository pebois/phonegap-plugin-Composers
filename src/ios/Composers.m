...
{
  Class testClass = (NSClassFromString(@"MFMessageComposeViewController"));
  if (testClass != nil) {
    if (![testClass canSendText]) {
...
{
  Class testClass = (NSClassFromString(@"MFMailComposeViewController"));
  if (testClass != nil) {
    if (![testClass canSendMail]) {
      CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR];
      [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
      return;
    }
